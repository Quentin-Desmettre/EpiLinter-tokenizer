...
void parse(const std::string & name, const std::string & src)
{
    ...
    if (src.empty() == false)
    {
        try
        {
            ...
            // Create an iterator on the file contents
            ...
            for ( ; it != end; ++it)
            {
                ...
                // Get the element's value/line/column/length
...
boost::wave::token_id tokenIdFromTokenFilter(const TokenFilter & filter)
{
    typedef std::map<TokenFilter, boost::wave::token_id>
        TokenFilterToIdMap;
...
CompiledFilterSequence prepareCompiledFilter(
    const FilterSequence & filterSeq)
{
    ...
    {
        FilterSequence::const_iterator it = filterSeq.begin();
        const FilterSequence::const_iterator end = filterSeq.end();
    ...
    const FilterSequence & filter)
{
    ...
    if (end != tokensInFile.end())
        std::cerr << "end differs" << std::endl;
